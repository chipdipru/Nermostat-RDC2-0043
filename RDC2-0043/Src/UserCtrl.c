...
void DEBOUNCE_TIMER_ISR(void)
{
  ...
  {
    if (Keys[key].State == PRESS_PENDING)     
    {
...
void KEYS_ISR(void)
{
  ...
  {    
    ...
    Keys[ActiveKey].Level = 0;
    Keys[ActiveKey].State = PRESS_PENDING;
...
void ENABLE_IN_ISR(void)
{
  ...
  Keys[ENABLE_IN].State = PRESS_PENDING;
...
//------------------------------------------------------------------------------
void UserCtrl_Init(void (*KeysCallback)(uint8_t), void (*EnableChangedCallback)(uint8_t))
{   
